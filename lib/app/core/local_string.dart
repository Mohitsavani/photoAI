class LocalString {
  static String title = "Photo AI";
  static String subtitle = "Sub Title";
  static String start = "Start";
  static String home = "Home";
  static String noInternet = "No Internet";
  static String noBeautificationPictures = "No Beautification Pictures";
  static String connectionIntro =
      "You are not connected to the internet Make sure Wi-Fi is on, Flight mode is off and try again.";
  static String checkConnection =
      "Check your internet connection and try again.";
  static String pleaseTurnOn = "PLEASE TURN ON";
  static String wifi = "Wi-Fi";
  static String mobileData = "Mobile Data";

  static String aieffect = "AI Effect";
  static String free = "Free";
  static String privacy = "Privacy";
  static String rateMyApp = "Rate My App";
  static String share = "Share";
  static String help = "Help";
  static String termsAndConditions = "Terms And Conditions";
  static String profileView = "ProfileView";
  static String profileIsWorking = "ProfileView is working";
  static String resultView = "ResultView";
  static String selectImage = "Select Image";
  static String suggested = "Suggested";
  static String gallery = "Gallery";
  static String example = "Example";
  static String imagePreview = "Image Preview";
  static String noImageSelected = "No image selected";
  static String editPicture = "Edit Picture";
  static String done = "Done";
  static String noDataFound = "No Data Found";
}

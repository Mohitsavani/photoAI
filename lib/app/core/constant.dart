import 'assets.dart';

final List<Map<String, dynamic>> exampleData = [
  {
    'Image':
        'https://images.unsplash.com/photo-1711725851054-2da29dae6206?q=80&w=1664&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  },
  {
    'Image':
        'https://plus.unsplash.com/premium_photo-1683749805319-2c481ae54bc1?q=80&w=1615&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1707419705861-db1b08dc36c3?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1709577938593-1fc3991d3c93?q=80&w=1548&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
  },
];

final List<Map<String, dynamic>> freeDataEditList = [
  {'icon': AppIcons.imageEnhance, 'name': 'Image Enhance'},
  {'icon': AppIcons.repair, 'name': 'Repair'},
  {'icon': AppIcons.baby, 'name': 'Baby'},
  {'icon': AppIcons.old, 'name': 'Old'},
  {'icon': AppIcons.portraitBeautification, 'name': 'Portrait Beautification'},
  {'icon': AppIcons.colorizer, 'name': 'Colorizer'},
  {'icon': AppIcons.backgroundRemover, 'name': 'Background Removal'},
  {'icon': AppIcons.strechRepair, 'name': 'Stretch Repair'},
  {'icon': AppIcons.correction, 'name': 'Correction'},
  {'icon': AppIcons.superHd, 'name': 'Super HD'},
];

final List<Map<String, dynamic>> freeDataList = [
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Image Enhance'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Repair'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Baby'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Old'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Portrait Beautification'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Colorizer'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Background Removal'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Stretch Repair'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Correction'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1712041503216-c466cee9c845?q=80&w=1587&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Super HD'
  },
];

final List<Map<String, dynamic>> aiEffectDataEditList = [
  {'icon': AppIcons.imageEnhance, 'name': '3D Cartoon Avatar'},
  {'icon': AppIcons.repair, 'name': 'Pixar Effects'},
  {'icon': AppIcons.baby, 'name': 'Cartoonized'},
  {'icon': AppIcons.old, 'name': 'Angel'},
  {'icon': AppIcons.portraitBeautification, 'name': 'Demon'},
  {'icon': AppIcons.colorizer, 'name': 'Open Eyes'},
  {'icon': AppIcons.backgroundRemover, 'name': 'Image Flow'},
  {'icon': AppIcons.strechRepair, 'name': 'Dynamic Zoom'},
  {'icon': AppIcons.correction, 'name': 'Animate'},
];

final List<Map<String, dynamic>> purchaseList = [
  {'icon': AppIcons.gems, 'name': '280 Images', 'Price': 'INR 270.00'},
  {'icon': AppIcons.gems, 'name': '580 Images', 'Price': 'INR 520.00'},
  {'icon': AppIcons.gems, 'name': '980 Images', 'Price': 'INR 800.00'},
];

final List<Map<String, dynamic>> aiEffectDataList = [
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': '3D Cartoon Avatar'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Pixar Effect'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Cartoonized'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Angel'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Demon'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Open Eyes'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Image Flow'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Dynamic Zoom'
  },
  {
    'Image':
        'https://images.unsplash.com/photo-1571816119607-57e48af1caa9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    'name': 'Animate'
  },
];
